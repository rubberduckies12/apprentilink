...
export const updateJobService = async (id, job_title, postcode, description, salary, field, apprenticeship_level, desired_education_level, start_date, match_message, close_message) => {
    ...
    if (job_title !== job.job_title) {
        const existingJobs = await getJobsByCompanyIdService(job.company_id);
        if (existingJobs.some(job => job.job_title === job_title))
            throw new AppError(400, "A job with this title has already been posted by this company. Cannot update.");
    }

    if (postcode !== job.postcode) {
        if (!validatePostcode(postcode))
            throw new AppError(400, "Invalid postcode format.");
    }

    const result = await pool.query("UPDATE jobs SET job_title=$1, postcode=$2, description=$3, salary=$4, field=$5, apprenticeship_level=$6, desired_education_level=$7, start_date=$8, match_message=$9, close_message=$10 WHERE id=$11 RETURNING *",
